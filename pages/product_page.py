...
class ProductPage(BasePage):
    ...
    def check_product_name(self):
        alert_text = self.browser.find_element(
            *ProductPageLocators.BOOK_MESSAGE).get_attribute("innerText")
        product_name = self.browser.find_element(
            *ProductPageLocators.BOOK_NAME).get_attribute("innerText")
        assert alert_text in product_name, f"Product name should be: {product_name}, contains : {alert_text}"

    def check_product_price(self):
        alert_text = self.browser.find_element(
            *ProductPageLocators.PRICE_MESSAGE).get_attribute("innerText")
        product_price = self.browser.find_element(
            *ProductPageLocators.BOOK_PRICE).get_attribute("innerText")
